...
public class GlobalExceptionHandler {
    ...
    @ResponseBody
    public ResultDO<String> requestMethodArgumentNotValid(MethodArgumentNotValidException ex) {
        return result(ErrorConstant.BAD_REQUEST.getCode(), ex.getBindingResult().getAllErrors().get(0).getDefaultMessage(), ex);
    }

    //400错误
    @ExceptionHandler({ValidationException.class})
    @ResponseBody
    public ResultDO<String> requestValidation(ValidationException ex) {
        return result(ErrorConstant.BAD_REQUEST.getCode(), ex.getMessage(), ex);
    }
